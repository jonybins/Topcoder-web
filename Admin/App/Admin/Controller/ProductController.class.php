...
class ProductController extends PublicController{
	...
	public function index(){
		...
		//搜索变量
		$is_show=$this->htmlentities_u8($_GET['tuijian']);
		$img_name=$this->htmlentities_u8($_GET['name']);
		//===============================
		// 产品列表信息 搜索
		//===============================
		//搜索
		$is_show!=='' ? $where.="del<1 AND is_show=$is_show" : null;
		if($is_show)
		{
		$img_name!='' ? $where.=" AND img_name like '%$img_name%'" : null;
		}else{
		$img_name!='' ? $where.="img_name like '%$img_name%'" : null;
		}
		define('rows',5);
		...
		$productlist=M('image')->where($where)->order('addtime')->limit($limit,rows)->select();
		foreach ($productlist as $k => $v) {			
			$productlist[$k]['brand'] = M('brand')->where('id='.intval($v['path']))->getField('name');
		}
		//==========================
		// 将GET到的数据再输出
		//==========================
	...
	public function add(){	
		...
		try{
			...
			$array=array(
				...
				'updatetime'=>time(),
				'user'    =>$_SESSION['admininfo']['name'],
				'content'=>$_POST['content'] , 
				'path' =>intval($_POST['brand_id']),
			...
			if ($up_arr) {	
					...
					foreach ($up_arr as $key => $value) {
					    ...
					    if(is_array($res)) {
					    	// 上传成功 获取上传文件信息保存数据库
					    	$array['src_image'] = 'UploadFiles/'.$res['savepath'].$res['savename'];
							$array['addtime']=time();
					    	$all_in[] = $array;
			...
			if(intval($id)>0){
				...
				$sql = M('image')->where('id='.intval($id))->save($array);
			}else{
				$sql = M('image')->addAll($all_in);
				$id=$sql;
			}
		...
		// 查询产品信息
		//=========================
		$pro_allinfo= $id>0 ? M('image')->where('id='.$id)->find() : "";
		//商场信息
		...
		if ($pro_allinfo['photo_string']) {
			$img_str = explode(',', trim($pro_allinfo['src_image'],','));
	...
	*/
	public function img_del(){
		...
		$check_info = M('image')->where('id='.intval($pro_id).' AND del=0')->find();
		if (!$check_info) {
			echo json_encode(array('status'=>0,'err'=>'产品不存在或已下架删除！'));
			exit();
		}

		// $arr = explode(',', trim($check_info['src_image'],','));
		...
		// 		}
		// 	}
		if($check_info['src_image']){
			$data = array();
			$data['src_image'] ='';
			$res = M('image')->where('id='.intval($pro_id))->save($data);
			if (!$res) {
	...
	public function set_new(){
		$pro_id = intval($_REQUEST['pro_id']);
		$tj_update=M('image')->where('id='.intval($pro_id).' AND del=0')->find();
		if (!$tj_update) {
		...
		$up = M('image')->where('id='.intval($pro_id))->save($data);
		// var_dump($up);
		if ($up) {
			echo json_encode(array('status'=>1));
			exit();
	...
	public function del()
	{
		...
		$info = M('image')->where('id='.intval($id))->find();
		if (!$info) {
		...
		$data['del_time'] = time();
		$up = M('image')->where('id='.intval($id))->save($data);
		var_dump($data);	
		if ($up) {
			$this->redirect('index',array('page'=>intval($_REQUEST['page'])));
			exit();
